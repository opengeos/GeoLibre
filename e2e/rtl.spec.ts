 ...
 * direction and language, the map staying pinned ltr, accessibility of the
 * mirrored chrome, and the data path working unchanged. Locators are
...
test("mirrors the document and loads a layer in the Arabic locale", async ({
  ...
  // The map subtree opts out of the mirror so MapLibre's physically anchored
  // controls don't half-flip (see the .maplibregl-map rule in index.css).
  await expect(page.locator(".maplibregl-map")).toHaveCSS("direction", "ltr");
