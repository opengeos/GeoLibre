...
#
# This file is shared by the Linux store/repo channels (Fedora COPR today,
# Flathub/Snap later): it gives the app a name, description, screenshots, and a
# release history in software centers. Only the <release> entry changes per
# version, so this script stamps the version and date and emits the rest
# verbatim.
#
# Usage:
...
# used by the native packages; Flatpak passes app.geolibre.GeoLibre).
set -euo pipefail
