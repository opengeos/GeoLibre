#!/usr/bin/env node
// Check that every portal listed by the US State GIS and US Local GIS panels
// still answers with datasets. City and county portals move and get renamed
// more often than anything else the app links to, and a dead entry fails
// quietly (an empty list or "Could not search"), so run this before a release
// or when a user reports a portal as broken.
//
...
const sets = [
  ...(which === "local" ? [] : US_STATE_GIS_CATALOGS.map((set) => ["state", set])),
  ...(which === "state" ? [] : US_LOCAL_GIS_CATALOGS.map((set) => ["local", set])),
];
