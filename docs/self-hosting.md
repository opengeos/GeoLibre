...
| `GEOLIBRE_GEOLENS_URL` | unset for same-origin, or your GeoLens URL | Pre-fills and automatically connects the GeoLens plugin. When unset, the plugin uses the browser origin, which suits a GeoLens API co-located behind the same reverse proxy. The last successful server is remembered per browser. |
