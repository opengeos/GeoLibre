...
| `GEOLIBRE_GEOLENS_URL` | unset for same-origin, your GeoLens URL, or `off` | Pre-fills and automatically connects the GeoLens plugin. When unset, the plugin uses the browser origin, which suits a GeoLens API co-located behind the same reverse proxy. `off` keeps the panel idle until the user chooses a server. The last successful server is remembered per browser. |
