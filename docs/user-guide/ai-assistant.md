...
| **Model Builder** | Creates a validated, editable Model Builder workflow from a description using the client-side vector algorithms, saves it with the project, and opens it for review before you run it. Raster and Whitebox tools cannot be placed in an assistant-built model; ask the assistant to run one directly, or reach for the Processing menu. If the canvas holds unsaved work or a run is still in flight, Model Builder asks before replacing it. |
