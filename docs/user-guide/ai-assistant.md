...
| **Model Builder** | Creates a validated, editable Model Builder workflow from a description using the client-side vector algorithms (raster and Whitebox tools are not available here), saves it with the project, and opens it for review before you run it. If the canvas holds unsaved work or a run is still in flight, Model Builder asks before replacing it. |
