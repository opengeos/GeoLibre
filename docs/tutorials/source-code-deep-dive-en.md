...
- **To strip all external CDN references** (for deployments that cannot reference any untrusted CDN), set `GEOLIBRE_NO_EXTERNAL_CDN=1`. This implies all `*_CDN=0` flags and additionally disables features that embed CDN URLs in their source code (storymap HTML export, built-in detection models, ONNX WASM, 3D Tiles decoder paths, and the Pyodide default URL). Third-party npm packages may still contain internal CDN URL strings that are not under GeoLibre's control
