...
- **Engine-specific strengths**: MapLibre is the default and has the broadest plugin support; Mapbox supplies Mapbox styles, globe, and terrain; Cesium is the native globe for terrain, 3D Tiles, CZML, ion, and I3S; ArcGIS renders native ArcGIS services in 2D maps and 3D scenes. Mapbox requires an access token, while Cesium and ArcGIS work without credentials and use optional credentials for provider content. See the [Mapbox](mapbox-renderer.md) and [ArcGIS](arcgis-renderer.md) references.
