...
- Raster menu with hillshade, slope, aspect, reproject, resample, clip by extent, clip by mask layer, polygonize, contour, zonal statistics, raster calculator, reclassify, mosaic, and focal statistics
    ...
    - Plus in-browser extraction of COG, WMS, and XYZ bounding-box subsets, a global DEM downloader under Processing → GeoLibre Toolbox → Raster (OpenTopography COP30, NASADEM, SRTM, COP90, ALOS, and SRTM15+; current-view or drawn extent), and a normalized-difference index builder for any HTTP COG
