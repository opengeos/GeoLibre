...
So each of these CDN engines needs the network on **first** use, then works offline. To remove even the first-use dependency, point `VITE_PYODIDE_INDEX_URL` at a same-origin Pyodide mirror, and build with `GEOLIBRE_PGLITE_CDN=0` and `GEOLIBRE_CEREUS_CDN=0` to vendor PGlite/PostGIS and the CereusDB wasm back into the build (PGlite alone re-adds ~22 MB to the Tauri binary). `gdal3.js` — which backs the Georeferencer's client-side GeoTIFF/COG export — is the exception: its wasm (~28 MB) and data (~12 MB) are never vendored, and `GEOLIBRE_GDAL_CDN=0` opts out of the CDN rather than bundling them, which leaves the loader with no paths and makes that export unavailable. The **desktop** build ships no service worker, so it re-fetches every one of these from jsDelivr on first use per install — unless PGlite/PostGIS and CereusDB are bundled with the flags above. Pyodide can be pointed at a mirror but is always fetched, and `gdal3.js` cannot be bundled at all.
