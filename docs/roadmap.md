...
## v3.0: Cesium as a first-class rendering engine, dynamic 3D scenes on the globe, ArcGIS write-back, and editable SVG print layouts
...
## v3.1: Mapbox and ArcGIS join the rendering engines, a live God's Eye View globe, OAuth sign-in for sharing, and a voice-driven AI Assistant (current)

- [x] **Four interchangeable rendering engines**: alongside MapLibre and Cesium, View → Rendering engine now offers **Mapbox GL JS** and the **ArcGIS Maps SDK for JavaScript** (loaded on demand from a CDN), and the app remembers the last engine across launches. The layer control, default map controls, map-area overlays, GPS, review comments, georeferencing, and sample markers mount on every engine. See [Rendering engines](user-guide/rendering-engines.md)
- [x] **The Mapbox renderer reaches parity**: PMTiles, MBTiles, LiDAR, 3D Tiles, Zarr, Deck.gl layers, 3D models, and DuckDB layers all render on it; marker icons, fill patterns, inverted fills, line decorations, geometry generators, heatmap and cluster renderers, label de-duplication, and data-defined label overrides port across; Identify covers WMS, pixel, and DuckDB layers with hover tips and photo popups; and the Plugins menu, Print Layout atlas, Layer Swipe, Street View, Overture Maps, GeoAgent, Geo Editor, and the Flight Simulator's free camera all run there. See [Mapbox renderer](mapbox-renderer.md)
- [x] **ArcGIS renders in 2D maps and 3D scenes**: native ArcGIS services plus PMTiles, MBTiles, Zarr, and NetCDF grids; 3D scenes with terrain and extrusion, scene zoom and bounds limits, and a nautical scale bar; icons, patterns, and labels on vector tiles; inverted fills, geometry generators, line decorations, and label overrides; raster effects and blend modes; identify popups, hover map tips, geotagged-photo popups, selection, and story-map fades; and the Print Layout atlas. Data plugin controls adapt to the ArcGIS engine. See [ArcGIS renderer](arcgis-renderer.md)
- [x] **A God's Eye View plugin turns the Cesium globe into a live situation room**: USGS earthquakes and CelesTrak satellite orbits as live CZML, keyless NASA FIRMS active fires, public transit from Entur and other providers, public CCTV snapshots and traffic cameras from Austin, New South Wales, Ontario, and DriveBC, TomTom live traffic flow, and AIS vessels, with in-plugin API key entry for the feeds that need one. See [Plugins](user-guide/plugins.md)
- [x] **Sign in with OAuth to share and publish**: the self-hostable server API gains an OAuth Authorization Code flow with S256 PKCE and scoped, expiring personal tokens; the web build signs in for project Share and the Gallery, and the desktop app adds OAuth sign-in with session management. Shares take access roles (view, comment, edit), link expiry, password protection, and revocation. See [Server API](server-api.md)
- [x] **A faster, more capable AI Assistant**: a voice command mode, a sub-second fast path for simple map commands through TypeSafe, an **OpenRouter** provider, live model discovery for Google, Anthropic, OpenAI, and Bedrock, explicit class breaks for `apply_symbology` with the applied stops reported back, plugin tools loaded on demand once there are more than twelve, plugin-contributed system-prompt guidance, and semantic search over the Whitebox catalog that ranks exact tool names first. See [AI Assistant](user-guide/ai-assistant.md)
- [x] **New data plugins**: **Fields of the World** agricultural field boundaries colored by model confidence, **Satellite Embeddings** for browsing and downloading pre-computed foundation-model embeddings such as AlphaEarth and Tessera, **IGN LiDAR HD** for French national point clouds, and an OSM downloader web service
- [x] **Cesium keeps growing**: Zarr layers render on the 3D globe through zarr-cesium, DXF and DWG drawings keep their Z values in 3D, the basemap picker offers Cesium and Mapbox basemaps, and the Ion quick picks add Google Photorealistic 3D Tiles and Asset Depot samples
- [x] **More ways in**: a `?stac=` link opens a STAC catalog or API collection, `?data=` opens LiDAR point clouds, the OGC API - Features form fills its bounding box from the map view, raster TileJSON metadata imports through XYZ sources, and the desktop app draws WMS tiles from servers that do not offer EPSG:3857. A runtime-configured deployment service library lets a hosted build ship its own curated services. See [Adding data](user-guide/adding-data.md)
- [x] **Styling and layer refinements**: continuous raster color ramps take opacity classes, popups have a configurable width and image height, a layer group's contents sort A to Z or Z to A, a style can be pasted from the layer menu and the Style panel, and the Buffer tool can dissolve its result
- [x] **For developers and notebooks**: `getLayers`/`onLayersChanged` on `GeoLibreAppAPI`, `moveLayerGroupToGroup` and `listLayerGroups` on the plugin API, a GeoLens connection that is remembered and auto-connects, and a Python API that arms Identify, toggles controls, and sets the projection. See [Plugin API](plugin-api.md) and [Python API](python.md)
- [x] **Hebrew joins the shipped locales** as a second right-to-left language, and every catalog is complete again after filling 351 missing keys. See [Internationalization](i18n.md)
