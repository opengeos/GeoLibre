...
- [x] **The globe carries its own chrome**: Reset view, fullscreen, and a scene-mode picker for 3D, 2D, and Columbus views (the flat modes in Web Mercator to match the project map), a native basemap and terrain gallery with Cesium Ion imagery plus keyless Esri, OpenStreetMap, and Stadia styles, cursor coordinates and elevation in the shared status bar, and controls that move between corners like their 2D counterparts
