...
export function useProjectFileActions(mapControllerRef: MapControllerRef) {
  ...
  const [qgisImportWarnings, setQgisImportWarnings] = useState<
    QgisProjectImportWarning[] | null
  >(null);
  ...
  const handleImportQgisProject = async () => {
    ...
    try {
      ...
      if (isTauri()) {
        ...
        for (const raster of imported.rasters) {
          try {
            ...
            console.error(
              `Failed to import QGIS raster "${raster.name}"`,
              error
            );
            imported.warnings.push({
              layerName: raster.name,
      ...
      setQgisImportWarnings(
        imported.warnings.length > 0 ? imported.warnings : null
      );
    } catch (error) {
      ...
      setActionError(
        error instanceof Error
          ? error.message
          : t("toolbar.error.couldNotImportQgisProject")
      );
    }
  };
  ...
  const openProjectFromShareUrl = async (
    ...
    try {
      ...
      if (options.authToken) {
        ...
        project = await resolveProjectXyzLayers(
          result.project,
          controller.signal
        );
      }
