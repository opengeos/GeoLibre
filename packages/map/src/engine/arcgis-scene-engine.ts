...
export function isArcGIS3DSupportedLayerType(layer: GeoLibreLayer): boolean {
...
export class ArcGISSceneController {
  private engine: ArcGISSceneEngine;

  constructor(engine: ArcGISSceneEngine) {
    this.engine = engine;
  }
  ...
  setStyle(basemapStyleUrl: string) {
    // Optionally update style url if it corresponds to an ArcGIS style
  }
...
export class ArcGISSceneEngine implements MapEngine {
  ...
  getController() {
    return new ArcGISSceneController(this);
  }
  ...
  readView(): MapViewState {
    ...
    if (center) {
      ...
      lat = center.latitude;
    } else if (this.view.camera?.position) {
      lng = this.view.camera.position.longitude;
      lat = this.view.camera.position.latitude;
    }

    let bearing = this.view.camera?.heading ?? 0;
  ...
  async syncLayers(layers: GeoLibreLayer[]): Promise<void> {
    ...
    for (const layer of layers) {
      if (!isArcGIS3DSupportedLayerType(layer) || !isSupported(layer)) {
        continue;
      }
  ...
  supportsLayer(layer: GeoLibreLayer): boolean {
    return isArcGIS3DSupportedLayerType(layer);
  }
