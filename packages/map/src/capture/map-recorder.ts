import type { MapCaptureResult, MapEngineClient } from "../engine/types";

/**
 * Records the live map to a video file through the MapEngine capture port.
 *
 ...
 *   fixed window.
 *
 * Every frame is obtained from the active engine, composited (and cropped, for a
 * selected area) into an offscreen canvas, and sampled by `MediaRecorder` via
 * `captureStream`. The intermediate canvas makes the crop and overlays possible:
 * a raw engine surface cannot be cropped or combined with DOM overlays directly.
 *
...
export async function recordMapCanvas({
  ...
  const initialCssWidth = client.viewport.getRect()?.width ?? initialCapture.canvas.clientWidth;
  const rect = computeCaptureRect(
    region ?? null,
    initialCapture.width,
    initialCapture.height,
    initialCssWidth || initialCapture.width,
  );
  if (!rect) {
  ...
  const deviceScale = initialCssWidth > 0 ? initialCapture.width / initialCssWidth : 1;
  ...
  // The compositor pulls a fresh engine snapshot every animation frame. The
  // adapter owns its renderer's redraw/canvas policy; this recorder only crops
  // the engine-neutral capture result and therefore works for a future ArcGIS
  // screenshot adapter too.
  let startedAt = 0;
  let lastSeconds = -1;
  let rafId = 0;
  let captureInFlight = false;
  const drawCapturedFrame = (capture: MapCaptureResult): void => {
    const source = capture.canvas;
    const liveCssWidth = client.viewport.getRect()?.width ?? source.clientWidth;
    const frameRect = computeCaptureRect(
      region ?? null,
      capture.width,
      capture.height,
      liveCssWidth || capture.width,
    );
    if (!frameRect) return;
    ...
    try {
      ...
      if (!recorderFailed) {
        recorderFailed = true;
        failRecording(
          new Error("Recording failed: the map capture is unreadable.", { cause: error }),
        );
      }
      return;
    }
    ...
    const baseRect = client.viewport.getRect();
    if (overlays.length > 0 && baseRect) {
      const overlayScale = liveCssWidth > 0 ? capture.width / liveCssWidth : 1;
  ...
  const drawFrame = () => {
    if (!captureInFlight && !recorderFailed) {
      ...
      void client.viewport
        .capture()
        .then(drawCapturedFrame)
        .catch((error: unknown) => {
          if (!recorderFailed) {
            recorderFailed = true;
            failRecording(
              new Error("Recording failed: could not capture the map frame.", { cause: error }),
            );
          }
        })
        .finally(() => {
