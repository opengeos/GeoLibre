import type { MapEngineClient, Unsubscribe } from "../engine/types";
import type { FeatureCollection, Geometry, Position } from "geojson";

/**
 * Records an animated camera "tour" across a sequence of keyframes to a video
 * file through the MapEngine capture and camera ports. Recording moves the
 * camera between keyframes while a `MediaRecorder` samples an offscreen canvas;
 * the result is a WebM blob the caller saves to disk.
 */
...
/**
 * Fly to one keyframe over `durationMs` and resolve when the engine reports
 * idle, with a timeout fallback. The duration is the source keyframe's outgoing
 * transition, passed in by the caller rather than read off the target keyframe.
 */
async function flyToKeyframe(
  client: MapEngineClient,
  kf: TourKeyframe,
  durationMs: number,
  signal?: AbortSignal,
  ...
  client.camera.applyView(kf, { mode: "fly", durationMs: duration });
  try {
    await client.camera.whenIdle({
      timeoutMs: duration + Math.max(500, duration * 0.25),
      signal,
    });
  } catch (error) {
...
export async function recordTour({
  ...
  const initialCapture = await client.viewport.capture();
  const canvas = document.createElement("canvas");
  canvas.width = initialCapture.width;
  canvas.height = initialCapture.height;
  const context = canvas.getContext("2d");
  if (!context || typeof canvas.captureStream !== "function") {
    throw new TourRecordingUnsupportedError();
  }
  context.drawImage(initialCapture.canvas, 0, 0, canvas.width, canvas.height);
  ...
  // instead of finishing the current segment. The local signal also lets a
  // capture failure terminate a hold without waiting for its full duration.
  const stopController = new AbortController();
  const stopOnAbort = () => {
    stopController.abort();
    client.camera.applyView(client.camera.readView(), { mode: "jump" });
  };
  ...
  const finished = new Promise<Blob>((resolve, reject) => {
    ...
    recorder.onerror = (event) => {
      ...
      recorderFailed = true;
      stopOnAbort(); // halt the in-flight move so the loop stops promptly
      reject(
  ...
  const paintCapture = (
    capture: Awaited<ReturnType<MapEngineClient["viewport"]["capture"]>>,
  ): void => {
    try {
      context.drawImage(capture.canvas, 0, 0, canvas.width, canvas.height);
    } catch (error) {
      captureError = new Error("Recording failed: the map capture is unreadable.", {
        cause: error,
      });
      recorderFailed = true;
      stopOnAbort();
  ...
  const pump = () => {
    if (!captureInFlight && !recorderFailed) {
      captureInFlight = true;
      void client.viewport
        .capture()
        .then(paintCapture)
        .catch((error: unknown) => {
          captureError = new Error("Recording failed: could not capture the map frame.", {
            cause: error,
          });
          recorderFailed = true;
          stopOnAbort();
    ...
    if (!recorderFailed) rafId = requestAnimationFrame(pump);
  };
  ...
  let restoreInteractions: Unsubscribe = () => undefined;
  try {
    restoreInteractions = client.interactions.suspendNavigation();
    ...
    client.camera.applyView(first, { mode: "jump" });
    ...
    for (let i = 0; i < keyframes.length; i++) {
      if (stopController.signal.aborted || recorderFailed) break;
      await delay(Math.max(0, keyframes[i].holdMs), stopController.signal);
      if (i < keyframes.length - 1 && !stopController.signal.aborted && !recorderFailed) {
        await flyToKeyframe(
          client,
          keyframes[i + 1],
          keyframes[i].transitionMs,
          stopController.signal,
        );
      }
    }

    if (!stopController.signal.aborted && !recorderFailed) {
      onProgress?.(1);
    }
  } finally {
    restoreInteractions();
