...
let nationalMapPosition: MapControlPosition = "top-left";
...
export const maplibreNationalMapRuntime: MapLibreHostedRuntime = {
  activate: (context, { position }) => {
    ...
    const added = context.addControl?.(nationalMapControl, nationalMapPosition) ?? false;
    if (!added) {
  ...
  deactivate: (context) => {
    ...
    context.removeControl?.(nationalMapControl);
    nationalMapControl = null;
  },
