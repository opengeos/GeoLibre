...
let esriWaybackPosition: MapControlPosition = "top-left";
...
/** Adapter-private MapLibre runtime for the Historical Imagery descriptor. */
export const maplibreEsriWaybackRuntime: MapLibreHostedRuntime = {
  activate: (context, { position, collapsed }) => {
    if (position) esriWaybackPosition = position;
    ...
    const added = context.addControl?.(esriWaybackControl, esriWaybackPosition) ?? false;
    if (!added) {
      detachStoreSync(esriWaybackControl);
      esriWaybackControl = null;
      return false;
    }
    restoreHostedControlPanel(esriWaybackControl, collapsed);
    setTimeout(() => {
  ...
  deactivate: (context) => {
    ...
    detachStoreSync(esriWaybackControl);
    context.removeControl?.(esriWaybackControl);
    esriWaybackControl = null;
