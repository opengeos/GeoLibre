...
function setMapillaryLabels(next: Partial<MapillaryLabels>): void {
  labels = { ...labels, ...next };
...
let externalLayerHost: MapEngineExternalLayerHost | null = null;
let floatingPanelHost: MapEngineFloatingPanelHost | null = null;
...
let panelPosition: "top-left" | "top-right" | "bottom-left" | "bottom-right" = "top-left";
...
function removeCoverage(activeMap: MapLibreMap): void {
  externalLayerHost?.unregister(LINES_STORE_LAYER_ID);
  externalLayerHost?.unregister(POINTS_STORE_LAYER_ID);
  for (const id of [
    SELECTED_LAYER_ID,
...
function showImage(imageId: string): void {
  ...
  floatingPanelHost?.open(PANEL_ID);
}
...
const floatingPanelRegistration: MapEngineFloatingPanelRegistration = {
  id: PANEL_ID,
...
function applyRuntimeState(value: unknown): boolean {
  ...
  const state = value as { labels?: Partial<MapillaryLabels> };
  if (!state.labels || typeof state.labels !== "object") return false;
  setMapillaryLabels(state.labels);
  return true;
}

function activateMapillaryRuntime(
  context: MapLibreHostedRuntimeContext,
  input: MapLibreHostedRuntimeActivation,
): boolean {
  const activeMap = context.map;
  if (!activeMap) return false;
  map = activeMap;
  floatingPanelHost = input.floatingPanelHost ?? null;
  externalLayerHost = input.externalLayerHost ?? null;
  applyRuntimeState(input.state);
  if (input.position) panelPosition = input.position;
  ensureCoverageWhenReady(activeMap);
  attachInteractions(activeMap);
  styleLoadHandler = () => {
    if (!map) return;
    map.once("idle", () => map && addCoverage(map));
  };
  activeMap.on("style.load", styleLoadHandler);
  (floatingPanelRegistration as { position?: typeof panelPosition }).position = panelPosition;
  unregisterPanel = floatingPanelHost?.register(floatingPanelRegistration) ?? null;
  floatingPanelHost?.open(PANEL_ID);
  return true;
}

function deactivateMapillaryRuntime(context: MapLibreHostedRuntimeContext): void {
  ...
  externalLayerHost = null;
}

export const maplibreMapillaryRuntime: MapLibreHostedRuntime = {
  activate: activateMapillaryRuntime,
  deactivate: deactivateMapillaryRuntime,
  setPosition: (_context, position) => {
    panelPosition = position;
    (floatingPanelRegistration as { position?: typeof position }).position = position;
    if (unregisterPanel && floatingPanelHost) {
      unregisterPanel = floatingPanelHost.register(floatingPanelRegistration);
    }
    return true;
  },
  applyState: (_context, state) => applyRuntimeState(state),
};
