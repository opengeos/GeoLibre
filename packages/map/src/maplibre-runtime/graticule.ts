...
/**
 * MapLibre-only coordinate graticule runtime.
 *
 ...
 */
function setGraticuleLabels(next: Partial<GraticuleLabels>): void {
  labels = { ...labels, ...next };
...
let rightPanelHost: MapEngineRightPanelHost | null = null;
...
function updateSettings(next: GraticuleSettings): void {
...
function publishState(): void {
  onStateChange?.({ settings: { ...settings } });
}

/** Update the adapter-private settings and report an interaction to the descriptor. */
function setGraticuleSettings(patch: Partial<GraticuleSettings>): void {
  updateSettings(normalizeGraticuleSettings({ ...settings, ...patch }));
  publishState();
}
...
class GraticuleControl implements IControl {
  ...
  onAdd(): HTMLElement {
    ...
    button.addEventListener("click", () => rightPanelHost?.open(PANEL_ID));
...
function applyRuntimeState(value: unknown): boolean {
  ...
  if (state.settings !== undefined) {
    ...
    if (!graticuleSettingsEqual(settings, next)) {
      updateSettings(next);
      changed = true;
    }
  }
  if (state.labels && typeof state.labels === "object") {
    setGraticuleLabels(state.labels);
    changed = true;
  }
  return changed;
}

function activateGraticuleRuntime(
  ...
  map = activeMap;
  rightPanelHost = input.rightPanelHost ?? null;
  ...
  // A style replacement drops native sources/layers. The adapter listens to its
  // own style lifecycle instead of asking a public plugin descriptor for a map.
  styleLoadHandler = () => {
    if (!map) return;
    cachedTextFont = null;
  ...
  if (!context.addControl?.(control, "top-right")) control = null;
  rightPanelHost?.open(PANEL_ID);
  return true;
}

function deactivateGraticuleRuntime(context: MapLibreHostedRuntimeContext): void {
...
/** Lazy adapter-private MapLibre implementation of the Gridlines control. */
export const maplibreGraticuleRuntime: MapLibreHostedRuntime = {
  activate: activateGraticuleRuntime,
  deactivate: deactivateGraticuleRuntime,
