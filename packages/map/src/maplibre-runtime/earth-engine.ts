...
const EARTH_ENGINE_CONTROL_POSITION = "top-left";
...
let earthEngineControlPromise: Promise<PluginControl> | null = null;
...
function closeEarthEngineControl(context: MapLibreHostedRuntimeContext): void {
  earthEngineStoreUnsubscribe?.();
  earthEngineStoreUnsubscribe = null;
  if (earthEngineControl && earthEngineControlMounted) {
    context.removeControl?.(earthEngineControl);
  }
...
async function openEarthEngineControl(
  context: MapLibreHostedRuntimeContext,
  token: number,
): Promise<boolean> {
  if (!earthEngineControl) {
    earthEngineControlPromise ??= createEarthEngineControl().finally(() => {
      earthEngineControlPromise = null;
    });
    earthEngineControl = await earthEngineControlPromise;
  }
  if (!active || token !== activationToken) return false;
  wireEarthEngineLayerSync(earthEngineControl);

  if (!earthEngineControlMounted) {
    const added = context.addControl?.(earthEngineControl, EARTH_ENGINE_CONTROL_POSITION) ?? false;
    if (!added) {
      earthEngineControl = null;
      earthEngineControlMounted = false;
      setEarthEngineControlVisible(false);
...
async function createEarthEngineControl(): Promise<PluginControl> {
  const { PluginControl } = await import("maplibre-gl-earth-engine");
  ...
  return new GeoLibreEarthEngineControl(earthEngineOptions());
}
...
/** MapLibre-only Earth Engine control/runtime behind the hosted-runtime seam. */
export const maplibreEarthEngineRuntime: MapLibreHostedRuntime = {
  activate: async (context, { onStateChange: nextOnStateChange }) => {
    onStateChange = nextOnStateChange ?? null;
    active = true;
    const opened = await openEarthEngineControl(context, ++activationToken);
    if (!opened) return false;
    // `PluginControl` creates its container during addControl(), so publish the
    // visible state synchronously for the toolbar and reveal/expand on the
    // following tick once its internal panel nodes are available.
    setEarthEngineControlVisible(true);
    return true;
  },
  deactivate: (context) => {
    active = false;
    ++activationToken;
    closeEarthEngineControl(context);
    onStateChange = null;
  },
  getState: () => ({ visible: earthEngineControlVisible }),
  runCommand: (_context, command) => {
    if (command !== "earth-engine.hide") return false;
    if (!earthEngineControl || !earthEngineControlMounted) return false;
