...
function attachEngine(context: MapLibreHostedRuntimeContext): boolean {
  ...
  else engine.applySettings(currentSettings);
  return true;
}
...
/** MapLibre-only atmosphere renderer behind the hosted-runtime seam. */
export const maplibreEffectsRuntime: MapLibreHostedRuntime = {
  activate: (context, { state }) => {
    currentSettings = normalizeEffectsSettings(state, DEFAULT_EFFECTS_SETTINGS);
    return attachEngine(context);
  },
