...
let basemapControlPosition: MapControlPosition = "top-left";
...
type ConfirmStyleReplace = (basemapName: string, count: number) => boolean;
type RasterBasemapDefinition = BasemapDefinition & {
  readonly source: Extract<BasemapDefinition["source"], { readonly type: "raster" }>;
};
...
/** Adapter-private MapLibre runtime for the Basemaps descriptor. */
export const maplibreBasemapControlRuntime: MapLibreHostedRuntime = {
  activate: (context, { position, collapsed, confirmStyleReplace }) => {
    if (position) basemapControlPosition = position;
    if (!basemapControl) {
      basemapControl = new BasemapControl(getBasemapControlOptions(confirmStyleReplace));
      basemapControl.on("basemapchange", (event) => {
        handleBasemapChange(event);
      });
      basemapControl.on("basemapremove", (event) => {
        handleBasemapRemove(event);
      });
    ...
    const added = context.addControl?.(basemapControl, basemapControlPosition) ?? false;
    if (!added) {
    ...
    const activeStyleId = getBasemapIdForStyleUrl(getActiveBasemap());
    ...
    restoreHostedControlPanel(basemapControl, collapsed);
  },
  deactivate: (context) => {
    ...
    context.removeControl?.(basemapControl);
    basemapControl = null;
  ...
  setPosition: (context, position) => {
    ...
    basemapControl.setState({
      activeBasemapId: getBasemapIdForStyleUrl(getActiveBasemap()),
    });
...
function getBasemapControlOptions(
  confirmStyleReplace: ConfirmStyleReplace | undefined,
  ...
  return {
    ...
    confirmStyleReplace: ({ basemap, replacedBasemapIds }) => {
      ...
      return confirmStyleReplace?.(basemap.name, count) ?? false;
    },
  };
}
...
function getActiveBasemap(): string {
  return useAppStore.getState().basemapStyleUrl;
}

function setActiveBasemap(styleUrl: string): void {
  useAppStore.getState().setBasemapStyleUrl(styleUrl);
}

function handleBasemapChange(event: BasemapControlEventPayload): void {
  ...
  if (source.type === "raster") {
    registerRasterBasemap(event.basemap, event);
    return;
  }
  ...
  if (!event.restored) {
    styleChangeFallback = {
      ...
      previousUrl: getActiveBasemap(),
    };
  }
  unregisterAllRasterBasemaps();
  setActiveBasemap(styleUrl);
}
...
function handleBasemapError(event: BasemapControlEventPayload): void {
  if (event.type !== "error") return;
  const fallback = styleChangeFallback;
  if (!fallback) return;
  styleChangeFallback = null;
  if (getActiveBasemap() !== fallback.attemptedUrl) return;
  if (fallback.previousUrl && fallback.previousUrl !== fallback.attemptedUrl) {
    setActiveBasemap(fallback.previousUrl);
  }
}

function handleBasemapRemove(event: BasemapControlEventPayload): void {
  // Narrows the BasemapControlEventPayload union so event.basemap is accessible.
...
function registerRasterBasemap(basemap: BasemapDefinition, event: BasemapChangeEvent): void {
  if (basemap.source.type !== "raster") return;
  const rasterBasemap = basemap as RasterBasemapDefinition;
  const managedRaster = getManagedRaster(event, basemap);
  if (!managedRaster) return;
  ...
  // nothing to do.
  if (event.mode !== "add") {
    unregisterRasterBasemapsExcept(basemap.id);
  } else if (registeredRasterLayers.has(basemap.id)) {
    return;
  }

  const store = useAppStore.getState();
  const existing = store.layers.find((layer) => layer.id === layerId);
  const layer = createRasterBasemapStoreLayer(rasterBasemap, managedRaster, existing);
  if (existing) store.updateLayer(layerId, layer);
  else store.addLayer(layer);
  registeredRasterLayers.set(basemap.id, layerId);
}

function createRasterBasemapStoreLayer(
  basemap: RasterBasemapDefinition,
  managedRaster: ManagedRasterBasemap,
  existing: GeoLibreLayer | undefined,
): GeoLibreLayer {
  const sourceId = managedRaster.sourceId;
  return {
    ...
    style: { ...DEFAULT_LAYER_STYLE, ...(existing?.style ?? {}) },
    metadata: {
      ...(existing?.metadata ?? {}),
    ...
    beforeId: managedRaster.beforeId ?? existing?.beforeId,
    geojson: existing?.geojson,
    sourcePath: existing?.sourcePath,
  };
}

function unregisterAllRasterBasemaps(): void {
  const store = useAppStore.getState();
...
function unregisterRasterBasemapsExcept(keepBasemapId: string): void {
  const store = useAppStore.getState();
