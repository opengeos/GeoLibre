...
const ANNOTATIONS_SOURCE_KIND = "annotation";
...
let annotationsPosition: MapControlPosition = "top-left";
 ...
 */
function setAnnotationLabels(next: Partial<AnnotationLabels>): void {
  labels = {
    ...labels,
    ...next,
...
type AnnotationRuntimeState = {
  readonly labels?: Partial<AnnotationLabels>;
};

function annotationRuntimeState(input: MapLibreHostedRuntimeActivation): AnnotationRuntimeState {
  const state = input.state;
  if (!state || typeof state !== "object" || Array.isArray(state)) return {};
  const labels = (state as { labels?: unknown }).labels;
  return labels && typeof labels === "object" && !Array.isArray(labels)
    ? { labels: labels as Partial<AnnotationLabels> }
    : {};
}

function applyAnnotationRuntimeState(input: MapLibreHostedRuntimeActivation): void {
  const { labels: nextLabels } = annotationRuntimeState(input);
  if (nextLabels) setAnnotationLabels(nextLabels);
}
...
function activateAnnotations(
  ...
  if (input.position) annotationsPosition = input.position;
  applyAnnotationRuntimeState(input);
  pluginActive = true;

  toolbarControl ??= new AnnotationToolbarControl();
  const added = context.addControl?.(toolbarControl, annotationsPosition) ?? false;
  if (!added) {
    toolbarControl = null;
    pluginActive = false;
    return false;
  ...
  if (context.map) bindMap(context.map);
  rediscoverAnnotationLayer();
  return true;
}

function deactivateAnnotations(context: MapLibreHostedRuntimeContext): void {
  setActiveTool(null);
  cancelTextInput();
  unbindMap();
  if (toolbarControl) {
    context.removeControl?.(toolbarControl);
    toolbarControl = null;
  }
...
function setAnnotationsPosition(
  context: MapLibreHostedRuntimeContext,
  ...
  annotationsPosition = position;
  context.removeControl?.(toolbarControl);
  if (context.addControl?.(toolbarControl, position)) return true;
  ...
  if (context.addControl?.(toolbarControl, previousPosition)) return false;
...
/** MapLibre-only annotation control/runtime behind the hosted-runtime seam. */
export const maplibreAnnotationsRuntime: MapLibreHostedRuntime = {
  activate: activateAnnotations,
  deactivate: deactivateAnnotations,
  setPosition: setAnnotationsPosition,
  applyState: (_context, state) => {
    applyAnnotationRuntimeState({ state });
    return true;
  },
};
