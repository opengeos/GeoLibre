...
let overturePosition: MapControlPosition = "top-left";
...
let onProjectStateChange: ((state: unknown) => void) | null = null;

function createOvertureControl(
  exportTextFile: ((filename: string, content: string) => void) | undefined,
): OvertureMapsControl {
  ...
  const control = new OvertureMapsControl({
    ...
    ...(exportTextFile
      ? {
          onExport: (filename, data) => exportTextFile(filename, JSON.stringify(data)),
        }
      : {}),
  });
  if (pendingState) {
...
/** Adapter-private MapLibre runtime for the Overture Maps descriptor. */
export const maplibreOvertureMapsRuntime: MapLibreHostedRuntime = {
  activate: (context, { position, collapsed, state, onStateChange, exportTextFile }) => {
    if (position) overturePosition = position;
    if (isOvertureMapsState(state)) pendingState = state;
    onProjectStateChange = onStateChange ?? null;
    if (!overtureControl) {
      overtureControl = createOvertureControl(exportTextFile);
      attachStoreSync(overtureControl);
    }
    const added = context.addControl?.(overtureControl, overturePosition) ?? false;
    if (!added) {
      detachStoreSync();
      overtureControl = null;
      return false;
    }
    restoreHostedControlPanel(overtureControl, collapsed);
    publishProjectState();
  },
  deactivate: (context) => {
    ...
    publishProjectState();
    detachStoreSync();
    context.removeControl?.(overtureControl);
    overtureControl = null;
    onProjectStateChange = null;
  },
  ...
  getState: () => overtureControl?.getState() ?? pendingState ?? undefined,
  applyState: (_context, state: unknown) => {
...
function handleControlEvent(control: OvertureMapsControl): void {
  publishProjectState(control.getState());
  if (syncing) return;
  syncing = true;
  try {
...
function publishProjectState(state = overtureControl?.getState() ?? pendingState): void {
  if (state !== null && state !== undefined) onProjectStateChange?.(state);
}
