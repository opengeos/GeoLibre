...
let nasaEarthdataPosition: MapControlPosition = "top-left";
...
export const maplibreNasaEarthdataRuntime: MapLibreHostedRuntime = {
  activate: (context, { position, collapsed }) => {
    ...
    const added = context.addControl?.(nasaEarthdataControl, nasaEarthdataPosition) ?? false;
    if (!added) {
  ...
  deactivate: (context) => {
    ...
    context.removeControl?.(nasaEarthdataControl);
    nasaEarthdataControl = null;
  },
