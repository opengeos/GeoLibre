...
const planetaryComputerControlPosition: MapControlPosition = "top-left";
...
let planetaryComputerActive = false;
let planetaryComputerActivationToken = 0;
...
async function ensurePlanetaryComputerControl(
  ...
  if (!planetaryComputerControlMounted) {
    const added =
      context.addControl?.(planetaryComputerControl, planetaryComputerControlPosition) ?? false;
    if (!added) {
 ...
 *
 * @param context - The adapter-private MapLibre runtime context.
 */
async function restorePlanetaryComputerLayers(
  context: MapLibreHostedRuntimeContext,
): Promise<void> {
  ...
  const control = await ensurePlanetaryComputerControl(context);
...
type PlanetaryComputerRuntimeRequest = {
  readonly openPanel?: boolean;
  readonly restoreLayers?: boolean;
};

function runtimeRequest(input: MapLibreHostedRuntimeActivation): PlanetaryComputerRuntimeRequest {
  const state = input.state;
  if (!state || typeof state !== "object" || Array.isArray(state)) return {};
  const request = state as Record<string, unknown>;
  return {
    openPanel: request.openPanel === true,
    restoreLayers: request.restoreLayers === true,
  };
}

async function activatePlanetaryComputerRuntime(
  context: MapLibreHostedRuntimeContext,
  input: MapLibreHostedRuntimeActivation,
): Promise<boolean> {
  const request = runtimeRequest(input);
  planetaryComputerActive = true;
  const activationToken = ++planetaryComputerActivationToken;
  const control = await ensurePlanetaryComputerControl(context, {
    hiddenOnMount: !request.openPanel,
  });
  if (
    !control ||
    !planetaryComputerActive ||
    activationToken !== planetaryComputerActivationToken
  ) {
    if (control && planetaryComputerControlMounted) {
      context.removeControl?.(control);
    }
    return false;
  }

  if (request.restoreLayers) {
    try {
      await restorePlanetaryComputerLayers(context);
    } catch (error) {
  ...
  if (request.openPanel) {
    setTimeout(() => {
      if (!planetaryComputerActive || activationToken !== planetaryComputerActivationToken) return;
...
function deactivatePlanetaryComputerRuntime(context: MapLibreHostedRuntimeContext): void {
  planetaryComputerActive = false;
  ++planetaryComputerActivationToken;
  ++planetaryComputerRestoreToken;
  const control = planetaryComputerControl;
  if (control && planetaryComputerControlMounted) {
    context.removeControl?.(control);
  }
  if (planetaryComputerControl === control) {
    resetPlanetaryComputerControl(control);
  }
}

/** MapLibre-only Planetary Computer control/runtime behind the hosted-runtime seam. */
export const maplibrePlanetaryComputerRuntime: MapLibreHostedRuntime = {
  activate: activatePlanetaryComputerRuntime,
  deactivate: deactivatePlanetaryComputerRuntime,
};
