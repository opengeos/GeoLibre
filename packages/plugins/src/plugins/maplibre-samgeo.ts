...
function appendCommon(form: FormData, req: SegmentationSnapshot, outputFormat: string): void {
...
function bboxOf(geometry: Geometry | null | undefined): [number, number, number, number] | null {
  ...
  let east = -Infinity;
  let north = -Infinity;
  mapPositions((geometry as { coordinates?: unknown } | null)?.coordinates, (p) => {
...
/**
 * Copy each detection's `score` onto the mask polygon it belongs to.
 *
 * The API's GeoJSON output is a vectorised mask raster whose only property is
 * the mask index, while its `detections` output carries the confidence score
 * per bounding box. Both are expressed in the raster CRS, so the mask polygon
 * whose extent best overlaps a detection box receives that box's score.
 */
export function attachDetectionScores(
  masks: FeatureCollection,
  detections: FeatureCollection,
): FeatureCollection {
  const boxes = detections.features
    .map((feature) => ({
      bbox: bboxOf(feature.geometry),
      score: (feature.properties as { score?: unknown } | null)?.score,
    }))
    .filter(
      (d): d is { bbox: [number, number, number, number]; score: number } =>
        d.bbox !== null && typeof d.score === "number",
    );
  if (!boxes.length) return masks;
  return {
    ...masks,
    features: masks.features.map((feature) => {
      const extent = bboxOf(feature.geometry);
      if (!extent) return feature;
      let best = 0;
      let score: number | undefined;
      for (const box of boxes) {
        const overlap = bboxOverlap(extent, box.bbox);
        if (overlap > best) {
          best = overlap;
          score = box.score;
        }
      }
      if (score === undefined) return feature;
      return {
        ...feature,
        properties: { ...(feature.properties ?? {}), score },
      };
    }),
  };
}
...
async function requestSegmentation(
  ...
  let result = await postSegmentation(endpoint, form, req.apiUrl, signal);
  if (req.mode === "text" && result.features.length) {
    // Only the text endpoint exposes per-object confidence, and only through
    // its `detections` format, so ask for those as a second request and join
    // the scores onto the mask polygons. A failure here costs the score
    // column, not the result.
    const detForm = new FormData();
    detForm.append("file", file, file.name);
    appendCommon(detForm, req, "detections");
    detForm.append("prompt", req.prompt.trim());
    detForm.append("backend", req.backend);
    detForm.append("confidence_threshold", String(req.confidence));
    try {
      result = attachDetectionScores(
        result,
        await postSegmentation(endpoint, detForm, req.apiUrl, signal),
      );
    } catch (error) {
