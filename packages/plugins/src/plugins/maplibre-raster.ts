...
// maplibre-gl-raster (re-verified against v0.10.0). All access is optional (?.)
 ...
 * maplibre-gl-raster (re-verified against v0.10.0) rejects non-tiled GeoTIFFs with a message
...
function createRasterControl(
  ...
  // _state.collapsed BEFORE emitting the event (re-verified against v0.10.0) --
...
function patchWebRasterOverlayFactory(
  ...
  // maplibre-gl-raster (re-verified against v0.10.0) calls `_deps.removeOverlay(this._map, this._overlay)`
